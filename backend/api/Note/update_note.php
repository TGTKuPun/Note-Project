<?php
    ...
    if (isset($data["note_id"]) && isset($data["note_title"]) && isset($data["note_desc"]) && isset($data["label_name"]) && isset($data["note_date"])) {
        ...
        $stmt->bind_param("ssisi", $note_title, $note_desc, $label_id, $note_date, $note_id);
        
        if ($stmt->execute()) {
            echo json_encode ([
                "note_id" => $note_id,
                "status" => "success",
                "message" => "Successfully update note to latest version",
            ]);
        }
        else {
            echo json_encode([
                "status" => "error",
                "message" => "Failed to update latest version of note to server",
            ]);
        }
